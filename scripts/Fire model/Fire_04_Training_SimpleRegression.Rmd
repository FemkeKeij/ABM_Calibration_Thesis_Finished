...
fit_data_time_full <- fit_data_time_full %>%
  mutate(datapoints = 'timepoints',
         summarise_runs = 'full')
```
...
fit_data_time_sum <- fit_data_time_sum %>%
  mutate(datapoints = 'timepoints',
...
fit_data_end_full <- fit_data_end_full %>%
...
fit_data_end_sum <- fit_data_end_sum %>%
  mutate(datapoints = 'endpoints',
         summarise_runs = 'summarised')
```
...
fit_all %>%
  ...
  separate(col = time_label,
           ...
           sep = '_') %>%
  write_csv('data/raw/fire_lr_predictions.csv')
```
...
```{python}
df_vals = pd.read_csv('data/raw/fire_lr_predictions.csv')
...
df_out.to_csv('data/raw/fire_lr_predictions_withoutputs.csv')
```
...
```{r}
errors2_data <- 
  read_csv('data/raw/fire_lr_predictions_withoutputs.csv')
errors1_data <- read_csv('data/raw/fire_lr_predictions.csv')
...
# Plots
```{r}
errors_long <- PrepErrorsForPlotting(errors)
```

% correctly predicted parameters (density & directions):
```{r}
plot1 <- PlotPercCorrectParams(errors_long)

ggsave('figures/fire_lr_perccorrect.pdf',
       height = 20, width = 20, units = 'cm')
```
RMSE, NRMSE, point prediction performance, and coverage for density parameter:
```{r}
plot2 <- PlotErrorsDensity(errors_long)

ggsave('figures/fire_lr_density_errors.pdf',
       height = 10, width = 20, units = 'cm')
```
F1, kappa score, and MCC for directions:
```{r}
plot3 <- PlotErrorsDirections(errors_long)

ggsave('figures/fire_lr_directions_errors.pdf',
